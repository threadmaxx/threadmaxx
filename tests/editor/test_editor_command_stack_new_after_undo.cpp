/// @file test_editor_command_stack_new_after_undo.cpp
/// @brief E3 — apply A, undo A, apply B → redo of A is no longer
/// possible; world reflects B.
...
int main() {
    ...
    // New command after an undo discards the redo deck.
    stack.execute(std::make_unique<SetX>(game.entity, 0.0f, 3.0f));
    engine.step();
    CHECK_EQ(xOf(engine.world(), game.entity), 3.0f);
    CHECK(!stack.canRedo());
