...
int main() {
    CHECK_EQ(std::string(threadmaxx::reflect::version_string()),
             std::string("0.9.0-dev"));
    CHECK_EQ(THREADMAXX_REFLECT_VERSION_MAJOR, 0);
    CHECK_EQ(THREADMAXX_REFLECT_VERSION_MINOR, 9);
    CHECK_EQ(THREADMAXX_REFLECT_VERSION_PATCH, 0);
    CHECK_EQ(THREADMAXX_REFLECT_VERSION, 0 * 10000 + 9 * 100 + 0);
    EXIT_WITH_RESULT();
}
