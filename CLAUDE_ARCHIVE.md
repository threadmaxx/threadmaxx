...
# 2026-05-31 distillation pass

Three sections of `CLAUDE.md` were trimmed on 2026-05-31 to keep the active
guide compact. The narrative material below was the verbatim pre-trim text
for the three sections; the active `CLAUDE.md` now carries a one-paragraph
contract + a pointer here / to `SHARDED_OPTIMISATION.md` for each.

## JobSystem — 2026-05-25 missed-wakeup fix (verbatim pre-trim narrative)

Workers park on per-worker CVs (one futex per worker keeps contention low at high pool sizes). The pre-2026-05-25 design had a missed-wakeup hole: when a worker was blocked in a user wait (e.g. `JobLatch::wait` inside a nested `parallelFor`), `submit()`'s `notify_one(target.cv)` was wasted, and other parked workers stayed parked because their predicate (`self.hasWork() || stopping_`) returned false on any wake whose work landed on a sibling queue. They re-parked without ever calling `trySteal`. Result: stranded work, intermittent ~1/5 hang on rpg_demo `--stress` at 71 workers. Three pieces close the hole without re-introducing contention:
...
## Sharded commit batches — verbatim pre-trim narratives

The active CLAUDE.md now carries a one-row-per-batch table. The full per-batch text below preserves the original rationale, empirical numbers, and edge-case discussion that the table compresses. Authoritative source for new work remains `SHARDED_OPTIMISATION.md`.
