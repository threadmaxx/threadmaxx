...
21. [Telemetry sinks](telemetry.md) — `ITraceSink` / `FileTraceSink`
    / `HudTraceSink`, the `FrameBudgetWatcher` built-in system, and
    the `setStallTimeout` watchdog (§3.7 batch 14).
22. [Serialization](serialization.md) — `World::snapshot()` plus the
